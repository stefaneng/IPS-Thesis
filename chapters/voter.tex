...
q(\eta, \delta) = 0
$$
For finite $S$ the configuration $\eta$ transitions to $\delta$ with probability
$$
P^\eta(\eta_t = \eta_x) = q(\eta, \delta) t + o(t)
$$
as $t \downarrow 0$.
...
The dual process for the voter model is a summary of \cite{steif91}.
...
The arrows from $x$ to $y$ represent the points of the Poisson process $\{N_{x,y}(t), t \geq 0\}$ represent when $y$ changes its value to the value at site $x$.
Then by Poisson superposition (Theorem \ref{thm:poisson_super}) we can combine all the $d$ Poisson processes at each site $x$ into one Poisson process with rate $1$.
The site $x$ waits exponential time with rate $1$ and then select one of its $\frac{1}{2d}$ neighbors to conform.
Then the voter model with initial state $\eta_0$ is $\{\eta_t^{\eta_0}\}$ where $\eta_t^{\eta_0}(x)$ is the value of the voter model at site $x$.
...
The process is $\{A_{x,t}(s) : 0 \leq s \leq t, x \in \Z^d\}$, which is a continuous time random walk with rate $1$ jumps with probability $\frac{1}{2d}$ to each of its neighbors.
For 2 sites $x, y \in \Z^d$ we have 
$\{A_{x,t}(s) : 0 \leq s \leq t, x \in \Z^d\}$ and $\{A_{y,t}(s) : 0 \leq s \leq t, y \in \Z^d\}$ that are independent until they meet.
...
\begin{proof}
Take $x, y \in \Z^d$ and let $\{A_{x,t}(s) : 0 \leq s \leq t, x \in \Z^d\}$ and $\{A_{y,t}(s) : 0 \leq s \leq t, y \in \Z^d\}$ be the random walks for a fixed time $t$.
...
Taking the limit as $t \to \infty$ the RHS goes to 0 by Theorem \ref{thm:rw_recurrent} since the random walk $\{A_{x,t}(s) - A_{y,t}(s) : 0 \leq s \leq t, x,y \in \Z^d\}$ is recurrent for $d = 1,2$.
\end{proof}
