...
Assume that we have a random walk on $\{0,1,\ldots, n\}$ with transition probabilities 
\begin{align*}
    i \to i - 1 &= \begin{cases}
        1 & i = n\\
        1 - p & i \in \{1,\ldots, n\}
...
Assume that we have a random walk with transition probabilities as defined in Lemma \ref{lem:rw_p_hit_zero} on $\{0,1,\ldots, k\}$ where 0 is absorbing and $k$ is reflecting as shown in Figure \ref{fig:rw_voter_model_discrete}.
...
\begin{proof}
Using Lemma \ref{lem:rw_p_hit_zero}, for state $i \in \{1, \ldots, n - 1\}$, the probability of never returning to state $i$, $\gamma_i$, is $1 - p$ time the probability of hitting 0 before $i$ when starting in state $i - 1$. That is,
$$
\gamma_i = (1 - p) \frac{
...
The limiting distribution of $E[N_{n}]^{-1} \cdot \tau_{C_n}$ is an exponential distribution with parameter 1.
\end{theorem}
...
Then we multiplying $\tau_{C_n}$ by 
$E[N_{n}]^{-1}$ such that by exponential scaling
\begin{align*}
...
$$
\frac{\lambda - 1}{\lambda^j - 1} \tau_{C_n} = E[N_{n}]^{-1} \tau_{C_n} \Rightarrow \exp(1)
$$
