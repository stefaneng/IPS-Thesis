...
\begin{figure}[h]
  \centering
   ...
   \caption{Simulation for contact process with $\lambda = 4$ (above the critical value) and periodic boundary conditions on $25 \times 25$ grid. Initial configuration is only one node with 1.}
...
\begin{figure}[h]
  \centering
   ...
   \caption{Simulation for contact process with $\lambda = 1.5$ (below the critical value) and periodic boundary conditions on $25 \times 25$ grid. Initial configuration has all nodes as ones.}
  \label{fig:contact_sim_torus_below_crit.png}
\end{figure}

\begin{defn}[Contact process]
Let $S = (V,E)$ be a graph where $V$ are the vertices (that have bounded degree) and $E$ is the set of edges.
...
Denote $\delta_0$, $\delta_1$ as the configurations with all zeros and all ones respectively.
\end{defn}

In Figure \ref{fig:contact_sim_torus_above_crit.png} we show one realization from a simulation of the contact process with periodic boundaries on a $25 \times 25 \subset \Z^2$ with $\lambda \geq \lambda_c$.
...
\begin{remark}
If the graph $S$ is finite, then eventually any initial configuration will eventually converge weakly to $\delta_0$.
\end{remark}

When the graph $S$ is infinite, then the limiting behavior of the system varies depending on $\lambda$.
The configuration of all zeros, $\delta_0$, is clearly a stationary distribution since it will always remain all zeros.

\begin{theorem} \cite{Liggett2002}
For a contact process on d-dimensional lattice $\Z^d$ there is a critical value $\lambda_c$ such that when
\begin{itemize}
    \item If $\lambda \leq \lambda_c$ then all distributions converge weakly to $\delta_0$.
    \item If $\lambda > \lambda_c$ then there is some other stationary distribution $\nu \not = \delta_0$ such that all distributions (except $\delta_0$) converge weakly to $\nu$.
\end{itemize}
Also, $\frac{1}{2d - 1} < \lambda_c < \frac{2}{d}$
\end{theorem}
...
\begin{figure}[h]
   ...
   \draw (2) edge[bend left] node[yshift=3mm]{rate $2$} (1);
...
\begin{figure}[h]
   ...
   \draw (3) edge[bend left] node[yshift=3mm]{rate $3$} (2);
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[start chain = going right,
   ...
   \draw (4) edge [bend left] node[yshift=3.5mm, xshift=-1mm]{rate $4$} (3);
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[start chain = going right,
   ...
   \draw (111) edge [bend left] node[yshift=3.5mm, xshift=-1mm]{rate $2$} (110);
...
If we want to simulate the behavior on $\Z^2$ then it is better to use \textit{periodic boundary conditions} where the boundaries wrap around to the other side (making the graph a torus).
