...
\subsection{Voter Model Dual Process}
...
\begin{align*}
    ...
    \end{cases}
\end{align*}
Where each of the rates are scaled by $(n - 1)$.
...
Let 
% The constant (n - 1) is admitted here. Maybe should add?
$$
X_i^{(i)} \sim \begin{cases}
  \exp\left(\displaystyle \frac{nk}{n - 1}\right) & i = k, N \text{ even}\\
  \exp\left(\displaystyle\frac{k (n - k)}{n - 1}\right) & i = k, N \text{ odd}\\
  \exp\left(2 i \displaystyle \frac{(n - i)}{n - 1}\right) & i \in \{1, \ldots, k-1\}
\end{cases}
$$
...
E[\tau_{C_n^{v}}] &= \begin{cases}
    \frac{(n - 1)}{n} + H_{n - 1} + H_{k + 1} & n \text{ even}\\
    \frac{(n - 1)}{n - k} + H_{n - 1} + H_{k + 1} & n \text{ odd}
\end{cases}\\
&\approx 0.69 n - 1.19
\end{align*}

where $H_m$ is the $m$-th harmonic number defined as
$$
