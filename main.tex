...
\begin{figure}[h]
  \centering
   ...
   \caption{Simulation for contact process with $\lambda = 4$ and periodic boundary conditions on $25 \times 25$ grid. Initial configuration is only one node with 1.}
  \label{fig:contact_sim_torus.png}
\end{figure}
...
In Figure \ref{fig:contact_sim_torus.png} we show one realization from a simulation of the contact process with periodic boundaries on a $25 \times 25 \subset \Z^2$.

\begin{remark}
If the graph $S$ is finite, then eventually any initial configuration will be all zeros.
\end{remark}

When the graph $S$ is infinite, then the limiting behavior of the system varies depending on $\lambda$.
Let $\delta_0$ be the configuration of all zeros.
This is clearly a stationary distribution since it will always remain all zeros.
