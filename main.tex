...
\begin{align*}
   ...
   \lambda +1}}-\frac{\left(-\sqrt{\lambda ^2+6 \lambda +1}+\lambda -1\right)
   ...
   e^{\frac{1}{2} \left(\sqrt{\lambda ^2+6 \lambda +1}-\lambda -3\right) x}}{2
   \sqrt{\lambda ^2+6 \lambda +1}}-\frac{\left(-\sqrt{\lambda ^2+6 \lambda
...
\begin{figure}
  \centering
    \includegraphics[width=.80\textwidth]{figures/contact_phase_densities.png}
   \caption{Density functions for $T_\lambda$ for $\lambda = 1, 2, 5, 15$ on the contact process with two nodes}
  \label{fig:contact_phase_densities}
\end{figure}

In Figure \ref{fig:contact_phase_densities} we plot the density from Equation \ref{eq:t_density} for various values of $\lambda$.
...
\subsubsection{Limiting Distribution}
TODO: Use R to simulate from the phase-type distribution
