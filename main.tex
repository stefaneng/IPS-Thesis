...
For all $i = 1,2,\ldots$ and $j = 1,2,\ldots, N_i$, let 

$$
X_i^{(j)} \sim \begin{cases}
  \exp\left(\displaystyle \frac{nk}{n - 1}\right) & j = k, N \text{ even}\\
  \exp\left(\displaystyle\frac{k (n - k)}{n - 1}\right) & j = k, N \text{ odd}\\
  \exp\left(2 i \displaystyle \frac{(n - j)}{n - 1}\right) & j \in \{1, \ldots, k-1\}
\end{cases}
$$
...
\begin{align*}
    ...
    &= k E[X^{(k)}] + (n - 1) \sum_{j = 1}^{k - 1} \frac{1}{n - i}\\
